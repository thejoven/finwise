...
- **升级手法** (符合 GFW 约定):
  ...
  - mastra: 205 host 上 `cd /opt/wiseflow/mastra && npm install` (npmmirror 源) + `systemctl restart wiseflow-mastra`
- **坑**: 别把 buildx 放后台就急着 `docker save` — 镜像还没出 → 空管道 → compose 退而去 registry 拉自建镜像撞 429. 必须等 build 完再 save. (升级期间旧 0.16.0 console 一直在跑, 无中断.)
- **验证**: `docker exec wiseflow-iii /app/iii --version`=0.16.1 healthy; console 容器内 binary 0.16.1, `GET :3113`=200; mastra `iii worker ready` (4 队列 + 5 http 路径).
...
| 项目目录 | `/opt/wiseflow/` |
| API 二进制 | `/opt/wiseflow/bin/wiseflow-api` |
| API service | `systemctl status wiseflow-api` |
| API log | `/var/log/wiseflow-api.log` |
| Mastra 路径 | `/opt/wiseflow/mastra/` (tsx 直跑, 不 build) |
...
# 或直接 ssh
ssh root@192.168.1.205 'tail -f /var/log/wiseflow-api.log'
```
...
# rsync + systemctl restart wiseflow-mastra + tail mastra log
```
...
# 服务管理
systemctl status wiseflow-api
systemctl restart wiseflow-api
journalctl -u wiseflow-api -f       # 也能跟 stdout
...
# 老的 wiseflow-iii.service systemd 已下线, 不要再用 systemctl
...
# 直连 postgres
docker compose exec postgres psql -U wiseflow -d wiseflow
...
go build -o /opt/wiseflow/bin/wiseflow-api ./cmd/api
systemctl restart wiseflow-api

# mastra service 管理
systemctl status wiseflow-mastra
systemctl restart wiseflow-mastra        # 改了 mastra/src/* 后用
tail -f /var/log/wiseflow-mastra.log     # 看 mastra 日志
# 改了 mastra/package.json 后:
cd /opt/wiseflow/mastra && npm install && systemctl restart wiseflow-mastra
```
...
systemd unit 在 `/etc/systemd/system/wiseflow-mastra.service`. ExecStart:
`/usr/local/bin/node /opt/wiseflow/mastra/node_modules/.bin/tsx /opt/wiseflow/mastra/src/index.ts`.
...
连同一个 iii engine 时会抢任务. 想本地 dev 时, 先 `ssh root@192.168.1.205 systemctl stop wiseflow-mastra`.
...
shadcn/ui 后台. 静态构建产物落 `/opt/wiseflow/web-admin/dist`, 由系统自带 nginx 在 `:8082` serving, 同时把 `/v1/*` / `/healthz` / `/metrics` 反代到 `127.0.0.1:8080`. 访问:
...
nginx vhost 模板在 `web-admin/deploy/wiseflow-admin.nginx.conf`. 改端口走环境变量 `ADMIN_PORT=9000 ./deploy/deploy.sh`.
...
- wiseflow-api: 已 `systemctl enable`, 跟随启动
- wiseflow-mastra: 已 `systemctl enable`, 跟随启动 (依赖 docker iii + docker postgres, 启动顺序由 systemd `After=docker.service` 处理)

iii 第一次起 (或换 named volume 后) 必做一次 chown, 否则 queue 持久化报权限:

```bash
ssh root@192.168.1.205 '
docker compose -f /opt/wiseflow/docker-compose.yml up -d iii
VOL=$(docker volume inspect wiseflow_iiidata -f "{{.Mountpoint}}")
chown -R 65532:65532 "$VOL"
docker compose -f /opt/wiseflow/docker-compose.yml restart iii
'
```
...
```bash
ssh root@192.168.1.205 'cd /opt/wiseflow && \
  systemctl stop wiseflow-mastra wiseflow-api && \
  docker compose down -v && \
  docker compose up -d postgres redis iii && \
  VOL=$(docker volume inspect wiseflow_iiidata -f "{{.Mountpoint}}") && chown -R 65532:65532 "$VOL" && \
  docker compose restart iii && \
  sleep 5 && \
  bash scripts/migrate.sh up && \
  systemctl start wiseflow-api wiseflow-mastra'
```
...
所以本地是空的 → 服务器上 `/opt/wiseflow/bin/wiseflow-api` 会被清掉. 跑 `--no-build` 之后必须紧跟一次 build:

```bash
ssh root@192.168.1.205 'cd /opt/wiseflow/server && /usr/local/go/bin/go build -o /opt/wiseflow/bin/wiseflow-api ./cmd/api && systemctl restart wiseflow-api'
```
...
`iii-console` 上游没出官方 docker 镜像, 我们自己用 [iii/Dockerfile.console](iii/Dockerfile.console) 装. 镜像在 `docker.io/wiseflow/iii-console:0.16.1` (本地 tag, 不推 registry). 跟一般 docker pull 不一样:
...
docker buildx build --platform linux/amd64 --load \
  -t wiseflow/iii-console:0.16.1 \
  -f iii/Dockerfile.console iii/

docker save wiseflow/iii-console:0.16.1 \
  | ssh root@192.168.1.205 'docker load'

ssh root@192.168.1.205 'cd /opt/wiseflow && docker compose up -d --force-recreate --no-build iii-console'
```

要升 console 版本: 改 `iii/Dockerfile.console` 里的 `ARG VERSION` 和 docker-compose `args.VERSION`, 然后跑上面三步.

> ⚠️ **别把 build 放后台就急着 `docker save`** — 镜像还没出 → 空管道 → load 失败 → compose 退而去 registry 拉自建镜像撞 429. 等 `buildx` 真的出镜像 (`docker images wiseflow/iii-console` 能看到目标 tag) 再 save. 验证 binary 版本对不对: `docker exec wiseflow-iii-console /usr/local/bin/iii-console --version`.
...
4. engine: `ssh 205 'cd /opt/wiseflow && docker compose pull iii && docker compose up -d iii'` → 等 healthy.
5. console: Mac buildx amd64 → save|load → `up -d --force-recreate --no-build iii-console` (见上).
6. mastra: `ssh 205 'cd /opt/wiseflow/mastra && npm install && systemctl restart wiseflow-mastra'` → 看 `/var/log/wiseflow-mastra.log` 出现 `iii worker ready`.
