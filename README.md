...
默认目标是 `root@192.168.1.205` (与现行 SERVER.md 一致, 可通过 `FLASHFI_HOST` 覆盖).
...
# 推到别的机器
FLASHFI_HOST=root@10.0.0.50 ./scripts/docker-deploy.sh
```
...
ssh root@target 'mkdir -p /opt/flashfi && chmod 700 /opt/flashfi'

# 服务器侧 .env 必须先手动创建一次 (含 LLM key + 强密码), 之后由 rsync 跳过.
ssh root@target 'test -f /opt/flashfi/.env || echo "需要先 scp .env.docker.example 模板填好上传"'
```
...
|---|---|---|
| API 进程 | `systemctl status flashfi-api` | `docker compose logs api` |
| Mastra | `systemctl status flashfi-mastra` | `docker compose logs mastra` |
...
A: 这里不需要 pull, 是本地 build. 检查 `docker compose --profile prod config` 看 image 字段是不是 `flashfi/*:local`. 如果是, 跑一次 `docker compose --profile prod build` 强制构建.
