#!/usr/bin/env bash
#
# fetch-fonts.sh — download the 8 bundled fonts into assets/fonts/.
#
# 英文正文/标题已改系统字体 (SF Pro / Roboto, 见 Text.tsx), 故不再拉 Source Serif 4.
# Playfair Display 仍保留 —— 仅 AlphaX 品牌字 / 报头副线 (`<Display serif>`) 在用.
#
# Sources (all China-reachable as of 2026-05):
#   - Playfair Display + Noto Serif SC: gwfh.mranftl.com
...
# 1) Playfair Display — AlphaX 品牌字 / 报头副线 (Latin only)
gwfh playfair-display latin "regular,italic,700,700italic" PlayfairDisplay

# 2) Noto Serif SC — body face, Simplified Chinese
gwfh noto-serif-sc chinese-simplified "regular,700" NotoSerifSC

# 3) JetBrains Mono — from official GitHub mirror
