 ...
 *
 * 实现: Reanimated 4 worklet 控制 translateX + opacity.
 *   - 抽屉宽 = screen * 0.85
...
export function CluesDrawer({ open, onClose, items, loading }: Props) {
  ...
  // translateX shared value: drawerWidth(关) ↔ 0(开). progress 派生: 0..1
  const tx = useSharedValue(drawerWidth);

  useEffect(() => {
    tx.value = withTiming(open ? 0 : drawerWidth, {
      duration: 260,
        ...
        <SafeAreaView edges={["top", "bottom", "right"]} style={styles.drawerInner}>
          <View style={styles.headerRow}>
            ...
            <TapEffect style={styles.closeBtn} onPress={onClose} disableEffect>
              <X size={18} color={theme.color.ink} strokeWidth={1.5} />
            </TapEffect>
          </View>
