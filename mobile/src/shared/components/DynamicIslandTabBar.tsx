...
/**
 * 底部栏 — 一颗悬浮的"tab 岛"玻璃胶囊 (液态玻璃).
 *
 * 形态: 一颗水平居中、脱离屏幕边缘悬浮的玻璃药丸, 内含五个 tab (订阅 · 早报 · 财知 · 标的 · 我)
 *   —— **上图标下文字**, 标签常驻可见.
 *
 * (分类切换已挪出底栏: 现贴在财知报头主名「财知」右侧, 见 HeaderCategoryCell —— 分类筛选只
 *   作用于财知内的信箱/降噪/归档/统计四张子页, 收进财知更合适, 底栏只管纯导航.)
 *
 * 切换特效 (克制, 单一主效果): 选中时一颗"透镜"高亮 (highlight) 用弹簧滑到选中格底下 ——
 *   tab 等宽 (TAB_WIDTH) 故位置可直接算: translateX = index × SLOT, 无需 onLayout 量. 这是
 *   **唯一**的切换动效 —— 底部 tab 是全 app 最高频的操作, 刻意不叠液态拉伸 / 图标弹跳 / 果冻
 *   按压, 一颗弹簧滑块利落带过 (高频交互, 动效越克制越不碍事).
 *
 ...
 *
 * 入场: 冷启 / 登录后胶囊从下方 18px 弹簧浮上来"落座" (只动 translateY 不动 opacity ——
 *   给 UIVisualEffectView 的父层动 alpha 会闪).
 *
 * 材质: iOS 26+ 走 GlassView (liquid glass), 以下 / Android 降级 BlurView —— 收在
 *   `./glass` 的 `TabBarGlass`. 图标/标签直接用 theme.color
 *   的动态色 (随明暗自动翻); 描边/高亮这类非调色板 rgba 走 `glassOverlay`.
 *
 * 为什么走自定义 `tabBar`: 悬浮胶囊要自控位置/形状/动画, 默认 bar 给不了. 自定义 tabBar 下,
 *   screenOptions 的 tabBarStyle / tint 等全部失效.
 *
...
const TAB_META: Record<string, TabMeta> = {
  ...
  // 早报: 拂晓/朝阳 (newspaper 已被订阅占用). 该 symbol 有 .fill 变体.
  morning: { icon: "sun.horizon", iconSelected: "sun.horizon.fill" },
...
/** 单个 tab —— 上图标下文字. 选中态靠图标/标签的 ink tint + 其下玻璃透镜滑入标示 (不做图标弹跳).
...
export function DynamicIslandTabBar({ state, navigation }: BottomTabBarProps) {
  ...
  // 入场: 胶囊从下方浮上来"落座" (弹簧轻微过冲 = 落下的分量感). 只动 translateY 不动
  ...
  useEffect(() => {
    ...
    enter.value = reduce
      ? 1
      : withDelay(60, withSpring(1, { damping: 15, stiffness: 150, mass: 0.8 }));
  }, [enter, reduce]);
  ...
  //   LENS_LEFT/LENS_TOP, 位移只剩 index × SLOT —— 这颗弹簧滑块是唯一的切换动效.
  //   grab = 横扫 (scrub) 时整颗透镜微微抬起, 像被手指拎着 (横扫交互的反馈, 非切换装饰).
  ...
  useEffect(() => {
    ...
    // 减少动态: 透镜直接落位 (滑动是前庭敏感项); 否则弹簧滑过去 —— 唯一的切换动效.
    activeX.value = reduce ? state.index : withSpring(state.index, SLIDE);
  }, [state.index, activeX, reduce]);
  const highlightStyle = useAnimatedStyle(() => ({
    transform: [
      { translateX: activeX.value * SLOT },
      { scale: grab.value }, // 横扫时整颗透镜微抬 (默认 1, 不影响静态/点按切换)
    ],
  }));
    ...
    () =>
      Gesture.Pan()
        ...
        .onStart(() => {
          ...
          // 减少动态: 跟手位移是直接操控 (保留), 但不做"被拎起"的抬升装饰.
          if (!reduce) {
        ...
        .onEnd((e) => {
          ...
          activeX.value = reduce ? target : withSpring(target, SLIDE);
        ...
        .onFinalize((_e, success) => {
          ...
          if (!success) {
            ...
            activeX.value = reduce ? stateIndex : withSpring(stateIndex, SLIDE);
          }
        }),
    [activeX, commitScrub, dragBase, grab, lastSnap, maxIndex, reduce, scrubTick, stateIndex],
  );
  ...
  return (
    ...
    >
      <Animated.View pointerEvents="box-none" style={[styles.row, enterStyle]}>
        {/* tab 岛 —— 两层叠放 (岛框只定尺寸/锚点):
            1) 玻璃胶囊层 (描边 + 玻璃 + 透镜): 其下的选中透镜随导航弹簧滑入选中格.
            2) tab 内容层 (图标/标签): 独立, 透明背景让其下玻璃/透镜透出.
               外面罩一层 Pan 手势 (GestureDetector) —— 横扫换台 (scrub), 见上.
            每个 tab 再各裹一层原生 ContextMenu —— 长按弹液态玻璃快捷菜单 (见 ./TabContextMenu). */}
        <Animated.View style={[styles.islandFrame, { width: islandWidth }]}>
          <Animated.View
            pointerEvents="none"
            style={[styles.capsuleVisual, { borderColor: overlay.border }]}
          >
            <TabBarGlass
              isDark={isDark}
...
const styles = StyleSheet.create({
  ...
  // 玻璃胶囊视觉层: 描边 + 圆角 + 把玻璃裁成药丸 (其下选中透镜随导航弹簧滑动).
  capsuleVisual: {
