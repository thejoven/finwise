---
name: flashfi-rn-native-feel
description: Make React Native apps for Flashfi Engine feel indistinguishable from a native iOS/Android app, while honoring the project's restraint philosophy. Use this skill when building or reviewing any RN UI in the Flashfi Engine project — capture screens, inbox, commitment cards, anxiety companion, timeline replay. Triggers on requests like "build a screen for X", "make this feel more native", "implement the signing page", "review my component", or any work touching Pressable, Haptics, modal presentation, platform differences, font loading. Also activates when editing files under src/features/ or app/ and when the conversation mentions iOS native-feel, Expo, SafeAreaView, scroll bounce, sheet modals, or platform adaptation.
---

# Flashfi Engine · React Native Native Feel Skill

> 让 Expo + RN 写的 Flashfi Engine, 在 iOS 上像 iOS, 在 Android 上像 Android,
> 但更重要的——**让它在每一个平台上, 都先像 Flashfi Engine**。

---

## 这份 Skill 是干什么的

Flashfi Engine 用 React Native (Expo SDK 53+) 写多端, 但产品哲学要求每一端都"原生感"。
...
这份 Skill 是 60 项 RN native-feel 检查清单 + 反主流的 Flashfi Engine 克制项,
...
- 编写或审查 Flashfi Engine 项目下的 RN UI 代码
...
### 原则 2 · Flashfi Engine 反主流的克制项

光做到"像 iOS"还不够。本项目还要**反主流移动应用的某些习惯**:

| 主流移动应用 | Flashfi Engine |
|---|---|
...
这些规则的依据全部在 `06_产品哲学.md` 里。详见 `references/05-flashfi-restraint.md`。
...
│   ├── 05-flashfi-restraint.md          # Flashfi Engine 专属克制 🟢
...
1. `references/05-flashfi-restraint.md`
...
MIT — 借鉴自 yetone/native-feel-skill 的结构, 内容为 Flashfi Engine 项目原创。
