...
> 作者视角:**后端工程师 (接口设计)**
> 这份文档回答:Flutter 调 Go 用什么接口、Mastra 调 Go 用什么接口、流式响应怎么处理、错误怎么标准化。

---

## 这份文档要回答的核心问题

1. 外部 API (Flutter → Go) 长什么样?
...
- `POST /v1/internal/rag/search` — 向量检索
- `POST /v1/internal/inferences` — 推演结果回写
- `POST /v1/internal/events` — 通用事件写入(Mastra 写回追问答案等)
- `GET /v1/internal/users/{id}/context` — 拉取用户档案快照

每个接口的鉴权(service token + signed JWT)、调用频率上限。
...
后台推演完成后,Flutter 怎么知道?三种方式备选:

- **推模型**:WebSocket 长连接(复杂,延迟最低)
- **拉模型**:Flutter 定时轮询 `/v1/signals/{id}` (简单)
